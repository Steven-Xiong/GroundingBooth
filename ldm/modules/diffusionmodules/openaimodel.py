...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context, objs, box_mask):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, SpatialTransformer):
                x = layer(x, context, objs, box_mask)  #box_mask区分fg和bg
            else:
                x = layer(x)
        return x
...
class UNetModel(nn.Module):
    ...
    def forward(self, input):
        ...
        # try:
        box_mask = input['layout']  # ['grounding_input']['ref_box']
        # except:
        #     import pdb; pdb.set_trace()
        #     print(input.keys())
        ...
        for module in self.input_blocks:
            h = module(h, emb, context, objs, box_mask)
            hs.append(h)

        h = self.middle_block(h, emb, context, objs, box_mask)  #或者二者一块学？(h, emb, context, emb_img, context_img, objs)

        for module in self.output_blocks:
            h = th.cat([h, hs.pop()], dim=1)
            h = module(h, emb, context, objs, box_mask)

        return self.out(h)   #这里加Mask?
