...
class DreamBoothDataset(BaseDataset_t2i):
    ...
    def get_sample(self, idx):
        ...
        image_name = [i for i in images if '.png' or 'jpg' in i][4] # or 'jpg'  4
        ...
        if object_name in OBJECT:
            ...
            input_prompt = KOSMOSG_OBJECT_PROMPTS[idx % 25] #改变一下顺序试一试？
        else:
            object_class = LIVE_OBJECT[object_name]
            prompt = LIVE_OBJECT_PROMPTS[idx % 25]
            input_prompt = KOSMOSG_LIVE_OBJECT_PROMPTS[idx % 25]
        ...
        # id2class = {v: k for k, v in class2id.items()}
        # class_key = id2class[folder_name]
